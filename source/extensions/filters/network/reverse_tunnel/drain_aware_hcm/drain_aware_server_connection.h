...
// observe a GOAWAY sent by the peer (the upstream tunnel-server client codec). The default server
...
public:
  ...
  void onGoAway(Http::GoAwayErrorCode error_code) override {
    ...
    if (!peer_goaway_handled_ && on_peer_goaway_ != nullptr) {
      ...
      ENVOY_LOG(info, "drain_aware_hcm: peer GOAWAY for connection (code={}); draining tunnel and "
                      "dialing replacement",
