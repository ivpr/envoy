 ...
 * a reverse tunnel. Scenarios 1 & 2: the downstream (HTTP server) emits GOAWAY when it drains or
 * rotates a tunnel; the upstream client codec calls back into these callbacks with onGoAway(),
 ...
 * a drain GOAWAY to the peer (scenario 3).
 */
...
public:
  ...
  DrainAwareClientConnection(Envoy::Http::ClientConnectionPtr inner,
                             ...
                             UpstreamCodecDrainRegistrySharedPtr registry, absl::string_view cluster,
  ...
  /**
   * Scenario 3: begin a graceful, two-phase drain from the upstream (client) side. Sends a
   * shutdown notice immediately (HTTP/2 GOAWAY with max stream id, so existing/in-flight streams
   * keep working) and a real GOAWAY after `drain_time`. Idempotent.
   */
