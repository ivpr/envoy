...
void ReverseConnectionIOHandle::onDownstreamConnectionClosed(const std::string& connection_key) {
  ...
  if (host_address.empty()) {
    // Key already removed (typically via markTunnelDrainingAndDialReplacement when the peer
    // sent GOAWAY earlier). Benign no-op; logged at debug to avoid noisy warnings.
    ENVOY_LOG(debug,
              "reverse_tunnel: Connection key {} already removed from tracking map; "
              "closure cleanup is a no-op",
              connection_key);
    return;
  }
...
void ReverseConnectionIOHandle::markTunnelDrainingAndDialReplacement(
  ...
  if (host_address.empty()) {
    // Already removed (e.g. host pruned by a DNS refresh, or a prior call). Benign no-op.
    ENVOY_LOG(debug, "Connection key {} not in tracking map; nothing to drain", connection_key);
    return;
  }

  // Drop the key so the maintenance loop sees a deficit and dials a replacement. The
  // underlying TCP socket is left alone: in-flight H2 streams keep running on it and
  // it closes naturally on the next FIN; onDownstreamConnectionClosed then no-ops.
  auto host_it = host_to_conn_info_map_.find(host_address);
  if (host_it != host_to_conn_info_map_.end()) {
    host_it->second.connection_keys.erase(connection_key);
    ENVOY_LOG(debug, "Removed draining connection key {} from host {} (remaining: {})",
              connection_key, host_address, host_it->second.connection_keys.size());
  }
  removeConnectionState(host_address, cluster_name, connection_key);

  // Defer maintenance to the next dispatcher pass instead of waiting for the periodic tick.
  if (rev_conn_retry_timer_) {
