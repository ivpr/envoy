...
public:
  ...
  /**
   * Key the parent IOHandle uses to track this tunnel (the local address of the outbound
   * TCP socket at handoff time). The drain-aware HCM passes this back to parent() when
   * the peer sends GOAWAY so the parent can drop the tunnel from tracking.
   */
  const std::string& connectionKey() const { return connection_key_; }

  /**
   * Parent ReverseConnectionIOHandle that owns this tunnel. Defensive nullptr return
   * if the parent has already been torn down.
   */
